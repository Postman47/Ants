...
class Solution:
    ...
    def __init__(self, depot, capacity, max_distance):
        ...
        self.max_distance = max_distance
        self.routes = []
        self._total_distance = 0 # Use a private attribute to store the total distance

    def add_route(self, route):
        """Adds a route to the solution."""
        self.routes.append(route)
        self._total_distance += route.calculate_total_distance(None) # Calculate and add
...
def update_pheromone(pheromone_matrix, solutions, evaporation_rate, customer_table, best_solution_so_far=None, elitism_factor=None):
    """Updates the pheromone matrix based on the solutions found by the ants, including elitism."""
    ...
    for solution in solutions:
        if is_solution_valid(solution, customer_table, solution.capacity, solution.max_distance):
    ...
    # Elitist deposit
    if best_solution_so_far and elitism_factor is not None:
        if is_solution_valid(best_solution_so_far, customer_table, best_solution_so_far.capacity, best_solution_so_far.max_distance):
            best_cost = best_solution_so_far.calculate_total_distance()
            if best_cost > 0:
                elite_pheromone_deposit = elitism_factor / best_cost
                for route in best_solution_so_far.routes:
                    path = [best_solution_so_far.depot] + route.customers + [best_solution_so_far.depot]
                    for i in range(len(path) - 1):
                        node1_id = path[i].id if isinstance(path[i], Customer) else 0
                        node2_id = path[i+1].id if isinstance(path[i+1], Customer) else 0
                        pheromone_matrix[node1_id][node2_id] += elite_pheromone_deposit
                        pheromone_matrix[node2_id][node1_id] += elite_pheromone_deposit # Assuming symmetric distances

def elitist_ant_system(depot, customer_table, capacity, max_distance, num_ants, alpha, beta, evaporation_rate, num_iterations, elitism_factor, optimal_value):
    """Runs the Elitist Ant System algorithm for CVRP with max distance constraint."""
    ...
    while ((best_distance > optimal_value) and (iteration < num_iterations)):
        ...
        update_pheromone(pheromone_matrix, all_ant_solutions, evaporation_rate, customer_table, best_solution, elitism_factor) # Pass customer_table here
